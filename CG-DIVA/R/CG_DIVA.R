...
# Created by: Institut f�r Diabetes-Technology Forschungs- und Entwichlungsgesellschaft mbH an der Universit�t Ulm
...
bootstrapping <- function(df,N_BS,seed,conf_level=0.95,version="")
{
  ...
  BCa <- function (dat, theta_h, a, qtl) 
  {
    ...
    if ((min(dat) >= theta_h) | (max(dat) <= theta_h) | is.infinite(a)){
